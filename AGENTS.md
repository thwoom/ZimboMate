...
Focus all work on the root application. Source lives directly under src/ (components, hooks, stores, lib utilities), Playwright specs and visual baselines live in 	ests/e2e/ and isual-*, and shared Tailwind tokens sit in src/index.css. Generated artifacts such as dist/, isual-*, screenshots/, and playwright-report/ must remain unedited. Legacy workspaces (	ask-manager, dungeon-world, .claude-screenshots) now live in C:\ZimboMateExternal for safekeeping.
