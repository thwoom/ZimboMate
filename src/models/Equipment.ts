...
export function parseTagString(tagString: string): Tag[] {
  ...
  for (const part of parts) {
    ...
    if (firstSpace > 0) {
      const name = part.slice(0, firstSpace) as ItemTag
      let value = part.slice(firstSpace + 1).trim()
...
export const COMMON_ITEMS: Array<
  Partial<Item> | Partial<Weapon> | Partial<Armor>
> = [
  // Weapons
  {
    name: 'Dagger',
