...
// Common item tags in Dungeon World
export type ItemTag = 
  // Weapon range tags
  | 'hand'     // Can be held in one hand
  | 'close'    // Arm's reach
  | 'reach'    // A few feet
  | 'near'     // Throwing range
  | 'far'      // Bow range
  ...
  | 'forceful' // Knocks back
  | 'messy'    // +1d4 damage but messy
  | 'piercing' // Reduces armor effectiveness
  | 'precise'  // Use DEX instead of STR
  | 'reload'   // Takes time to reload
  | 'stun'     // Can stun enemies
  | 'thrown'   // Can be thrown
  | 'two-handed' // Requires both hands
  
  // Armor tags
  | 'worn'     // Must be worn
  | 'clumsy'   // -1 ongoing to DEX-based moves
  
  // General tags
  | 'applied'  // Used by applying to something
  | 'awkward'  // Unwieldy or hard to use
  | 'dangerous' // Using it is risky
  | 'ration'   // Food that counts as a ration
  | 'requires' // Requires something else
  | 'slow'     // Takes time to use
  | 'touch'    // Requires touch
  | 'uses'     // Has limited uses
  | 'weight'   // Has weight value
  
  // Value tags
  | 'coins'    // Has coin value
