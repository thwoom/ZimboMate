...
import { Campaign, CampaignSession, JournalEntry, NPC, Location } from '../../models/Campaign';
...
const CampaignPanel: React.FC < PanelProps> = ({ id }) => {
  ...
  const handleCreateItem = (type: string, data: unknown) => {
  ...
  const handleUpdateItem = (type: string, itemId: string, updates: unknown) => {
    if (!selectedCampaign) return;
  ...
  const renderItemCard = (item: unknown) => {
    const handleEdit = () => {
      setPanelState(prev => ({
        ...prev,
      ...
      <div key={item.id} className="campaign-item-card">
        ...
        <div className="item-content">
          ...
          {panelState.selectedTab === 'journal' && (
            <>
              <p className="item-content-preview">{item.content.substring(0, 100)}...</p>
              <div className="item-meta">
                <span > Date: {new Date(item.date).toLocaleDateString()}</span>
...
const AddItemModal: React.FC<{
  ...
  onConfirm: (type: string, data: unknown) => void;
  onCancel: () => void;
...
const ItemDetailsModal: React.FC<{
  item: unknown;
  onUpdate: (type: string, itemId: string, updates: unknown) => void;
  onCancel: () => void;
          ...
          <>
            ...
            <div className="form-group">
              ...
              <input
                ...
                onChange={(e) => setFormData({ ...formData, xpGained: parseInt(e.target.value) || 0 })}
