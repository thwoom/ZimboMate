...
import { useGameStore, useCharacter, useInventory, useMoves } from './GameStore';
import { 
  ValidationResult,
  CharacterValidation,
  ItemValidation,
...
export const useValidatedCharacterUpdate = () => {
  ...
  const updateCharacterWithValidation = useCallback((updates: Partial<Character>) => {
    ...
    validation.warnings.forEach(warning => {
      console.warn('Character validation warning:', warning);
    });
    ...
    if (validation.valid || validation.errors.length < lastValidation?.errors.length!) {
      ...
      if (triggers.length > 0) {
        console.log('Auto-triggers detected:', triggers);
...
export const useValidatedItemOperations = () => {
  ...
  const addItemWithValidation = useCallback((item: Item) => {
    ...
    encumbranceValidation.warnings.forEach(warning => {
      console.warn('Encumbrance warning:', warning);
    });
  ...
  const equipItemWithValidation = useCallback((itemId: string) => {
    ...
    const equippedItems = Object.values(updatedItems).filter(i => i.equipped);
  ...
  const addMoveWithValidation = useCallback((move: Move) => {
    ...
    if (!reqValidation.valid) {
      reqValidation.errors.forEach(error => {
        console.error('Move requirement error:', error);
      });
      return false;
    }
...
export const useGameStateValidation = () => {
  ...
  useEffect(() => {
    ...
    if (result.errors.length > 0) {
      console.error('Game state validation errors:', result.errors);
    }
    if (result.warnings.length > 0) {
      console.warn('Game state validation warnings:', result.warnings);
    }
