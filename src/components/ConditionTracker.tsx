...
export const ConditionTracker: React.FC < ConditionTrackerProps> = ({
  ...
  const handleCreateCondition = useCallback((conditionData: unknown) => {
    if (!activeCharacterId) return;

    try {
      const newCondition = conditionService.createCondition({
        ...conditionData,
...
const ConditionCard: React.FC < ConditionCardProps> = ({
  ...
  const display = conditionService.getConditionDisplay(condition);
...
interface CreateConditionFormProps {
  onCreate: (conditionData: unknown) => void;
  onCancel: () => void;
}
          ...
          {formData.canStack && (
            <div className="condition-tracker__form-group">
              ...
              <input
                ...
                onChange={(e) => setFormData(prev => ({ ...prev, maxStacks: parseInt(e.target.value) || 1 }))}
                min="1"
                className="condition-tracker__input"
              />
            </div>
