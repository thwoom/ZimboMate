...
export const HoldManager: React.FC<HoldManagerProps> = ({
  ...
  const handleSpendHold = (
    ...
    if (success) {
      logger.info('Spent hold', { amount, optionId })
    }
  }
    ...
    return (
      <Card className={`${className} opacity-75`}>
        <CardContent className='p-4 text-center'>
          <div className='text-muted-foreground  text-sm'>No active holds</div>
  ...
  if (compact) {
    ...
    return (
      <div className={`flex items-center gap-2 ${className}`}>
        ...
        <span className='text-sm font-medium'>{totalHold} Hold</span>
                        ...
                        {Array.from({ length: hold.maxAmount }, (_, i) => (
                          <div
                            ...
                            className={`w-2 h-2 rounded-full ${
                              i < hold.amount ? 'bg-primary' : 'bg-gray-300 '
                            }`}
                          />
                        ))}
                      </div>
                          ...
                          {moveInfo.options.map((option) => (
                            ...
                            >
                              ...
                              <Button
                                ...
                                size='sm'
                                onClick={() =>
                                  handleSpendHold(hold, option.id, option.cost)
                                }
