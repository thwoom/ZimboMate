...
export const ImportPanel: React.FC<ImportPanelProps> = ({
  ...
  const startImport = useCallback(() => {
    const validFiles = importFiles.filter(
      (f) => f.status !== ImportStatus.ERROR,
    )
  ...
  const clearCompleted = useCallback(() => {
    setImportFiles((prev) =>
      prev.filter((f) => f.status !== ImportStatus.SUCCESS),
    )
  }, [])
  ...
  const getStatusIcon = (status: ImportStatus) => {
    switch (status) {
      case ImportStatus.SUCCESS:
        return <CircleCheck size={16} className='text-chart-2' />
      case ImportStatus.ERROR:
        return <TriangleAlert size={16} className='text-destructive' />
            ...
            <div className='flex items-center justify-between'>
              ...
              <div className='flex gap-2'>
                <Button
                  ...
                  disabled={
                    !importFiles.some((f) => f.status === ImportStatus.SUCCESS)
                  }
                >
                  Clear Completed
                </Button>
                <Button
                  ...
                  disabled={
                    operationInProgress ||
                    !importFiles.some((f) => f.status === ImportStatus.PENDING)
                  }
                >
                  Start Import
                </Button>
                    ...
                    <div className='flex items-center justify-between'>
                      ...
                      <div className='flex items-center gap-3'>
                        {(importFile.status === ImportStatus.VALIDATING ||
                          importFile.status === ImportStatus.IMPORTING) && (
                          <div className='w-24'>
                            <Progress
