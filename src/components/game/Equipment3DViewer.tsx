...
const resolveColorVariable = (variable: string, fallback: string) => {
...
const createEquipmentPalette = () => ({
  steel: resolveColorVariable('--primary-border', '#c0c0c0'),
  wood: resolveColorVariable('--parchment-700', '#8b4513'),
  leather: resolveColorVariable('--parchment-600', '#654321'),
  bone: resolveColorVariable('--parchment-200', '#f5f5dc'),
  iron: resolveColorVariable('--muted-foreground', '#696969'),
  potion: resolveColorVariable('--accent', '#ff6b6b'),
  gold: resolveColorVariable('--chart-4', '#ffd700'),
})

const useEquipmentPalette = () => {
  const [palette, refreshPalette] = React.useReducer(
    () => createEquipmentPalette(),
    null,
    () => createEquipmentPalette(),
  )
  ...
  useEffect(() => {
    ...
    const updatePalette = () => refreshPalette()
    updatePalette()
...
type EquipmentPalette = ReturnType<typeof useEquipmentPalette>

// 3D Item Model Component
const ItemModel3D: React.FC<{
  item: Item
  autoRotate: boolean
...
export const Equipment3DViewer: React.FC<Equipment3DViewerProps> = ({
  ...
  const palette = useEquipmentPalette()
            ...
            <div className='flex items-center justify-between'>
              <div className='flex items-center gap-2'>
                <Button
                  ...
                  className={`h-8 w-8 ${isAutoRotating ? 'text-primary' : 'text-muted-foreground'}`}
                >
                  <RotateCcw size={16} />
                </Button>
