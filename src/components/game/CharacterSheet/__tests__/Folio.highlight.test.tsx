...
vi.mock('../FolioSpellsPage', () => ({
  ...
  default: (props: { highlighted?: boolean }) => {
    mocks.spells(props)
    return (
      <div data-testid='spells-page' data-highlighted={props.highlighted} />
    )
  },
}))
  ...
  default: (props: {
    ...
    return (
      <div data-testid='notes-page' data-highlighted={props.highlighted}>
        <button
          type='button'
          onClick={() => props.onNoteCreated?.('Journal Entry')}
        >
          save-note
        </button>
      </div>
...
describe('folio highlight behaviour', () => {
  ...
  it('shows header label for stats highlight', () => {
    ...
    expect(mocks.header).toHaveBeenCalledWith(
      expect.objectContaining({ highlighted: true, focusLabel: 'HP check' }),
    )
  })
