...
export const RollHistoryWidget: React.FC<RollHistoryWidgetProps> = ({
  ...
  const sortedRolls = [...filteredRolls].sort(
  ...
  const stats = {
    ...
    successRate:
      rollHistory.length > 0
        ? (rollHistory.filter((r) => r.result === 'success').length /
            rollHistory.length) *
          100
        : 0,
  }
          ...
          <CardContent className='p-4'>
            <div className='flex items-center justify-between text-sm'>
              <div className='text-muted-foreground'>
                Last roll:{' '}
                {rollHistory.length > 0
                  ? formatTimestamp(rollHistory[0].timestamp)
                  : 'Never'}
              </div>
              <div className='flex items-center gap-2'>
                <TrendingUp size={14} />
                <span>{stats.successRate.toFixed(0)}% success rate</span>
              </div>
            </div>
          </CardContent>
        </Card>
