    ...
    >
      <CardContent>
        <div className='space-y-4'>
            ...
            <div className='flex-1'>
              <div className='flex items-center gap-3 mb-2'>
                ...
                {entry.isImportant && (
                  <Badge
                    variant='default'
                    className='importance-badge-high text-xs'
                  >
                    <Star size={12} />
                    Important
                  </Badge>
          ...
          {/* Content Preview */}
          <div>
            <p className={expanded ? '' : 'line-clamp-3'}>{entry.content}</p>
...
export const CampaignJournal: React.FC<CampaignJournalProps> = ({
  ...
  const [filterByImportance, setFilterByImportance] = useState<boolean | null>(
    null,
  )
  ...
  const isDeleteDialogOpen = entryToDelete !== null

  const campaign = useCampaignStore((state) => state.getCampaign(campaignId))
  const deleteJournalEntry = useCampaignStore(
    (state) => state.deleteJournalEntry,
  )
  ...
  const filteredAndSortedEntries = useMemo(() => {
    ...
    if (filterByImportance !== null) {
      entries = entries.filter(
        (entry) => entry.isImportant === filterByImportance,
      )
    }
  ...
  const confirmDeleteEntry = () => {
    ...
    deleteJournalEntry(campaignId, entryToDelete.id)
    setEntryToDelete(null)
  }

  const deleteDialogMessage = entryToDelete
    ? `This will permanently delete "${entryToDelete.title}".`
        ...
        <div className='flex items-center justify-between'>
          <div>
            ...
            <p className='text-muted-foreground'>
              {filteredAndSortedEntries.length} of
              {campaign.journal.length} entries
            </p>
          </div>
          <Button
            variant='primary'
          ...
          <CardContent>
            <div className='flex flex-wrap items-center gap-4'>
              ...
              >
                ...
                <option value={JournalSortBy.IMPORTANCE}>
                  Sort by Importance
                </option>
