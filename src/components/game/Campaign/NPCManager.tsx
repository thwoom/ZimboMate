        ...
        <div className='space-y-4'>
          ...
          <div className='flex items-start justify-between'>
            ...
            <div className='flex items-center gap-2'>
              <Button variant='ghost' size='sm' onClick={() => onEdit(npc)}>
                <Edit size={16} />
              </Button>
              <Button variant='ghost' size='sm' onClick={() => onDelete(npc)}>
                <Trash2 size={16} />
              </Button>
            </div>
          </div>
...
export const NPCManager: React.FC<NPCManagerProps> = ({
  ...
  const [filterByImportance, setFilterByImportance] = useState<
    NPCImportance | ''
  >('')
  const [filterByDisposition, setFilterByDisposition] = useState<
    NPCDisposition | ''
  >('')
  const [isModalOpen, setIsModalOpen] = useState(false)
  const [editingNPC, setEditingNPC] = useState<NPC | undefined>()
  const [sortBy, setSortBy] = useState<
    'name' | 'importance' | 'firstMet' | 'lastSeen'
  >('name')
    ...
    if (searchQuery) {
      ...
      npcs = npcs.filter(
        (npc) =>
          ...
          (npc.secrets &&
            npc.secrets.some((s) => s.toLowerCase().includes(query))),
      )
    }
        ...
        <div className='flex items-center justify-between'>
          <div>
            ...
            <p className='text-muted-foreground'>
              {filteredAndSortedNPCs.length} of
