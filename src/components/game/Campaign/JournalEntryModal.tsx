...
function validateJournalEntry(
  ...
  if (!trimmedContent) errors.content = 'Content is required'
...
export const JournalEntryModal: React.FC<JournalEntryModalProps> = ({
  ...
  const updateJournalEntry = useCampaignStore(
    (state) => state.updateJournalEntry,
  )

  const [newTag, dispatchNewTag] = useReducer(
  ...
  const handleSubmitForm = useCallback(
    async (formState: JournalEntryFormState): Promise<string> => {
      const trimmedData = {
        ...formState,
  ...
  const handleAddTag = useCallback(() => {
    if (!newTag.trim() || !canAddMoreTags) return

    if (addTag(newTag)) dispatchNewTag('')
            ...
            <CardHeader className='flex flex-row items-center justify-between'>
              <div>
                ...
                <p className='text-sm text-muted-foreground'>
                  Capture key campaign moments and tag them for quick discovery
                  later.
                </p>
              </div>
                ...
                <div className='space-y-2'>
                  <div className='flex items-center justify-between'>
                    ...
                    >
                      <Star
                        size={12}
                        className={state.isImportant ? 'fill-current' : ''}
                      />
