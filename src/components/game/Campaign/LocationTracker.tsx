        ...
        <div className='space-y-4'>
          ...
          <div className='flex items-start justify-between'>
            <div className='flex-1'>
              ...
              <div className='flex items-center gap-4 text-sm text-muted-foreground'>
                ...
                <div className='flex items-center gap-1'>
                  <Eye size={14} />
                  Visited {location.visited.length} time
          ...
          {location.visited.length > 0 && (
            <div className='flex items-center gap-2 text-sm text-muted-foreground'>
              ...
              Last visited{' '}
              {formatDateRelative(
                location.visited[location.visited.length - 1],
              )}
            </div>
          )}
          ...
          {location.dangers && location.dangers.length > 0 && (
            <div>
              ...
              <div className='flex flex-wrap gap-2'>
                {location.dangers.map((danger) => (
                  <Badge
                    key={`${location.id}-danger-${danger}`}
                    variant='secondary'
                    className='text-xs'
                    style={{
            ...
            <div>
              ...
              <div className='flex flex-wrap gap-2'>
                {location.resources.map((resource) => (
                  <Badge
                    key={`${location.id}-resource-${resource}`}
                    variant='secondary'
                    className='text-xs'
                    style={{
    ...
    if (searchQuery) {
      ...
      locations = locations.filter(
        (location) =>
          ...
          (location.dangers &&
            location.dangers.some((d) => d.toLowerCase().includes(query))) ||
          (location.resources &&
            location.resources.some((r) => r.toLowerCase().includes(query))),
      )
    }
        ...
        <div className='flex items-center justify-between'>
          <div>
            ...
            <p className='text-muted-foreground'>
              {filteredAndSortedLocations.length} of
              {campaign.locations.length} locations
            </p>
          </div>
          <Button
            variant='primary'
