...
function getDispositionIcon(disposition: string) {
  switch (disposition) {
    case 'friendly':
      return <Heart size={16} className='text-chart-2' />
    case 'neutral':
...
function getImportanceColor(importance: string) {
  switch (importance) {
    case 'high':
      return 'bg-destructive/20 text-destructive border-destructive/40/30'
    case 'medium':
      return 'bg-chart-4/120/20 text-chart-4 border-yellow-500/30'
    case 'low':
      return 'bg-muted/500/20 text-muted-foreground border-border/30'
    default:
      return 'bg-muted/500/20 text-muted-foreground border-border/30'
  }
}
...
export const NPCModal: React.FC<NPCModalProps> = ({
  ...
  const [newSecret, dispatchNewSecret] = useReducer(
  ...
  const handleSubmitForm = useCallback(
    async (formState: NPCFormState): Promise<string> => {
      const trimmedState = {
        ...formState,
  ...
  const handleAddSecret = useCallback(() => {
    if (addSecret(newSecret)) dispatchNewSecret('')
  }, [addSecret, newSecret])
            ...
            <CardContent>
              <form onSubmit={handleSubmit} className='space-y-6'>
                ...
                <div className='grid grid-cols-2 gap-4'>
                  ...
                  <div className='space-y-2'>
                    ...
                    <select
                      ...
                      onChange={(event) => {
                        const value = event.target
                          .value as NPCFormState['importance']
                        setState((prev) => ({
                          ...prev,
                          importance: value,
                        }))
                ...
                <div className='space-y-3'>
                  ...
                  {secrets.length > 0 && (
                    <div className='space-y-2'>
                      {secrets.map((secret) => (
                        <div
                          key={secret}
                          className='flex items-start gap-2 p-3 bg-muted/500/10 rounded-lg'
                        >
                  ...
                  <div className='flex gap-2'>
                    <Input
                      value={newSecret}
                      onChange={(event) =>
                        dispatchNewSecret(event.target.value)
                      }
                ...
                <div className='flex items-center justify-between pt-4'>
                  <div className='flex items-center gap-3'>
                    <div
                      className={`px-2 py-1 rounded text-xs border ${getImportanceColor(state.importance)}`}
                    >
                      {state.importance.charAt(0).toUpperCase() +
                        state.importance.slice(1)}{' '}
                      Importance
                    </div>
                    <div className='flex items-center gap-1 text-sm text-muted-foreground'>
                      {getDispositionIcon(state.disposition)}
                      {state.disposition.charAt(0).toUpperCase() +
                        state.disposition.slice(1)}
                    </div>
                  </div>
                  <div className='flex gap-3'>
                    <Button
