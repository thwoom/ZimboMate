import type { MonsterOrigin, MonsterTemplate, QuickMonster } from '../../models/Monster'
...
export const MonsterManager: React.FC<MonsterManagerProps> = ({
  ...
  const filteredMonsters = useMemo(() => {
    ...
    return fromSearch
  }, [favorites, getFavoriteTemplates, searchQuery, selectedOrigin, showFavoritesOnly, templates, searchTemplates])
    ...
    <div className="space-y-6">
      <Card variant="elevated">
        <CardHeader>
          ...
          <CardDescription>
            Browse official creatures, spin up quick foes, and drop them into the current encounter
          </CardDescription>
        </CardHeader>
      </Card>
        ...
        <CardContent className="space-y-4">
          ...
          <div className="grid gap-3">
            {filteredMonsters.slice(0, MAX_TEMPLATES_PREVIEW).map((monster) => {
              ...
              return (
                <div key={monster.id} className="rounded-lg border border-border/60 bg-background/60 p-4">
                  <div className="flex flex-wrap items-center justify-between gap-3">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      {showCombatIntegration && (
                        <Button size="sm" variant="primary" onClick={() => handleAddToCombat(monster)}>
                          Add to Combat
                        </Button>
                      )}
                    </div>
