...
### Steps to Reproduce
${userActions.length > 0 ? userActions.map((action, index) => `${index + 1}. ${action}`).join('\n') : '1. [Please describe the steps to reproduce]'}
...
## Recent User Actions
${this.getUserActionHistory().map(action => `- ${action}`).join('\n')}
...
${userActions.length > 0 ? userActions.map(action => `- ${action}`).join('\n') : '- No recent actions recorded'}
