...
const SpellCompendium: React.FC < SpellCompendiumProps> = ({
  ...
  const preparationValidation = useMemo(() => {
    if (!character || !showPreparationTools) return null;
  ...
  const getSpellLevelLabel = (level: SpellLevel): string => {
    if (level === 0) return 'Cantrip / Rote';
    return `Level ${level}`;
  };
  ...
  return (
    <div className="spell-compendium">
      ...
      <div className="results-summary">
        <span>{sortedSpells.length} spells found</span>
        {character && (
          <span>• {spellCompendiumService.getAvailableSpells(character).length} available for your class</span>
        )}
      </div>
        ...
        <div className="spell-comparison">
          ...
          <div className="comparison-grid">
            <div className="comparison-spell">
              ...
              <p><strong > Level:</strong> {getSpellLevelLabel(spellComparison.spell1.level)}</p>
              <p><strong > School:</strong> {getSpellSchoolLabel(spellComparison.spell1.school)}</p>
              <p><strong > Range:</strong> {spellComparison.spell1.range || 'N / A'}</p>
              <p><strong > Duration:</strong> {spellComparison.spell1.duration || 'N / A'}</p>
              <p><strong > Tags:</strong> {spellComparison.spell1.tags.join(', ')}</p>
            </div>

            <div className="comparison-spell">
              <h4>{spellComparison.spell2.name}</h4>
              <p><strong > Level:</strong> {getSpellLevelLabel(spellComparison.spell2.level)}</p>
              <p><strong > School:</strong> {getSpellSchoolLabel(spellComparison.spell2.school)}</p>
              <p><strong > Range:</strong> {spellComparison.spell2.range || 'N / A'}</p>
              <p><strong > Duration:</strong> {spellComparison.spell2.duration || 'N / A'}</p>
              <p><strong > Tags:</strong> {spellComparison.spell2.tags.join(', ')}</p>
            </div>
          </div>
          ...
          {spellComparison.similarities.length > 0 && (
            <div className="similarities">
              <h5 > Similarities:</h5>
              <ul>
                {spellComparison.similarities.map((similarity, index) => (
      ...
      {preparationValidation && (
        <div className={`preparation-validation ${preparationValidation.isValid ? 'valid' : 'invalid'}`}>
          ...
          <div className="validation-summary">
            <p><strong > Total Levels:</strong> {preparationValidation.totalLevels}/{preparationValidation.maxLevels}</p>
            <p><strong > Cantrips:</strong> {preparationValidation.cantripsCount}</p>
            <p><strong > Leveled Spells:</strong> {preparationValidation.leveledSpellsCount}</p>
          </div>
          ...
          {preparationValidation.errors.length > 0 && (
            <div className="validation-errors">
              <h4 > Errors:</h4>
              <ul>
                {preparationValidation.errors.map((error, index) => (
          ...
          {preparationValidation.warnings.length > 0 && (
            <div className="validation-warnings">
              <h4 > Warnings:</h4>
              <ul>
                {preparationValidation.warnings.map((warning, index) => (
        ...
        {sortedSpells.map(spell => (
          ...
          >
            ...
            <div className="spell-content">
              ...
              <p className="spell-effect"><strong > Effect:</strong> {spell.effect}</p>
