...
const STAT_ORDER: Array<keyof Attributes> = ['STR', 'DEX', 'CON', 'INT', 'WIS', 'CHA']
...
const getModifierDisplay = (value: number): string => (value >= 0 ? `+${value}` : `${value}`)
...
export const UnifiedRollSystem: React.FC<UnifiedRollSystemProps> = ({
  ...
  const character = useCharacterStore(
    state => state.characters.find(entry => entry.id === characterId) ?? null,
  )
        ...
        <div className="flex flex-wrap items-center justify-between gap-4">
          <div>
            ...
            <p className="text-sm text-muted-foreground">
              Roll stats, trigger core moves, and keep a focused log without the gimmicks.
            </p>
          </div>
          ...
          {QUICK_MOVES.map(move => (
            ...
            >
              <div className="flex items-start justify-between gap-3">
                <div>
                  <div className="text-sm font-semibold text-foreground">{move.label}</div>
      ...
      <section className="space-y-4">
        ...
        <div className="grid gap-3 rounded-lg border border-border bg-card/80 p-4 shadow-sm md:grid-cols-[2fr_1fr]">
          <div className="space-y-3">
            <div>
              <label className="text-xs font-medium text-muted-foreground" htmlFor="custom-roll-label">
                Label
              </label>
              <Input
            ...
            <div>
              <label className="text-xs font-medium text-muted-foreground" htmlFor="custom-roll-notes">
                Notes (optional)
              </label>
              <Input
          ...
          <div className="flex flex-col justify-end gap-3">
            <div>
              <label className="text-xs font-medium text-muted-foreground" htmlFor="custom-roll-modifier">
                Modifier
              </label>
              <Input
