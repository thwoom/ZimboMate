...
export class OptimizedCalculationEngine {
  ...
  private setupMemoization() {
    ...
    this.memoizedCalculations.set('attributeModifiers', this.memoize(
      (attributes: unknown) => this.calculateAttributeModifiers(attributes),
      (attributes: unknown) => JSON.stringify(attributes),
    ));

    // Memoize effective modifier calculations
    this.memoizedCalculations.set('effectiveModifiers', this.memoize(
      (modifiers: unknown, debilities: unknown) => this.calculateEffectiveModifiers(modifiers, debilities),
      (modifiers: unknown, debilities: unknown) => `${JSON.stringify(modifiers)}-${JSON.stringify(debilities)}`,
    ));
  ...
  calculateBatch(contexts: CalculationContext[]): CalculatedValues[] {
    ...
    contexts.forEach(context => {
      ...
      Object.values(context.inventory.items).forEach((item: unknown) => {
        if (item.equipped) {
  ...
  calculateSelective(
    ...
    if (changedFields.includes('inventory')) {
      ...
      const equippedItems = Object.values(context.inventory.items).filter((i: unknown) => i.equipped);
      result.currentLoad = Object.values(context.inventory.items).reduce(
        (sum: number, item: unknown) => sum + (item.weight * (item.quantity || 1)), 0,
      );
  ...
  private calculateAttributeModifiers(attributes: unknown): unknown {
    // This would need to be implemented or we could use the calculation engine
    return {};
  }

  private calculateEffectiveModifiers(modifiers: unknown, debilities: unknown): unknown {
    // This would need to be implemented or we could use the calculation engine
    return {};
  }
