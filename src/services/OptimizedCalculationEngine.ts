...
export class OptimizedCalculationEngine {
  ...
  private setupMemoization() {
    ...
    this.memoizedCalculations.set('effectiveModifiers', this.memoize(
      (modifiers: any, debilities: any) => this.calculateEffectiveModifiers(modifiers, debilities),
      (modifiers: any, debilities: any) => `${JSON.stringify(modifiers)}-${JSON.stringify(debilities)}`
    ));
    
    // Memoize armor calculations
    this.memoizedCalculations.set('armor', this.memoize(
      (baseArmor: number, items: any[], modifiers: any[]) => {
  ...
  private setCache<T>(key: string, value: T): void {
    ...
    if (this.cache.size > this.cacheMaxSize) {
      ...
      const sortedEntries = Array.from(this.cache.entries())
        .sort((a, b) => (a[1] as CacheEntry<any>).timestamp - (b[1] as CacheEntry<any>).timestamp);
  ...
  getCacheStats(): {
    ...
    const oldestEntry = entries.length > 0
      ? new Date(Math.min(...entries.map(e => (e as CacheEntry<any>).timestamp)))
      : null;
  ...
  private calculateAttributeModifiers(attributes: any): any {
    // This would need to be implemented or we could use the calculation engine
    return {};
  }
  
  private calculateEffectiveModifiers(modifiers: any, debilities: any): any {
    // This would need to be implemented or we could use the calculation engine
    return {};
  }
  
  private calculateTotalArmor(baseArmor: number, items: any[], modifiers: any[]): number {
    // This would need to be implemented or we could use the calculation engine
    return baseArmor;
  }
}
