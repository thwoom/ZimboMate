...
class ConditionService {
  ...
  getConditionsForCharacter(characterId: string, filter?: ConditionFilter): Condition[] {
    let conditions = Array.from(this.conditions.values())
  ...
  private loadConditions(): void {
    try {
      ...
      if (stored) {
        ...
        conditionsArray.forEach((condition: unknown) => {
  ...
  private loadNotifications(): void {
    try {
      ...
      if (stored) {
        ...
        notificationsArray.forEach((notification: unknown) => {
  ...
  private checkExpiringConditions(): void {
    const allConditions = Array.from(this.conditions.values());
