...
class XPIntegrationService {
  ...
  getAnalytics(characterId: string): XPAnalytics {
    ...
    entries.forEach((entry) => {
      xpBySource[entry.source.id] =
        (xpBySource[entry.source.id] || 0) + entry.amount
    })

    const xpBySession: Record<string, number> = {}
    entries.forEach((entry) => {
      if (entry.sessionId) {
        xpBySession[entry.sessionId] =
          (xpBySession[entry.sessionId] || 0) + entry.amount
      }
    })
    ...
    const mostCommonSourceId = Object.entries(xpBySource).sort(
      ([, a], [, b]) => b - a,
    )[0]?.[0]
    const mostCommonSource = mostCommonSourceId
      ? this.xpSources.get(mostCommonSourceId)!
      : this.xpSources.get('failure')!
    ...
    const xpTrend = entries
      ...
      .reduce(
        (trend, entry) => {
          const cumulative =
            (trend[trend.length - 1]?.cumulative || 0) + entry.amount
          trend.push({
  ...
  private createNotification(entry: XPEntry) {
    const notification: XPNotification = {
      ...
      actions:
        entry.source.category === 'failure'
          ? [
              {
                label: 'View Character',
                action: () => {
                  // Navigate to character sheet
                  logger.info('Navigate to character sheet')
                },
              },
            ]
          : undefined,
  ...
  private formatXPMessage(entry: XPEntry): string {
    const amount =
      entry.amount > 0 ? `+${entry.amount}` : entry.amount.toString()
      ...
      actions: [
        {
          label: 'Level Up Now',
          action: () => {
            logger.info('Trigger level up process')
          },
        },
        {
          label: 'View Character',
  ...
  importXPData(data: any): boolean {
    try {
      ...
      logger.error('Failed to import XP data:', error)
      return false
    }
  }
}
