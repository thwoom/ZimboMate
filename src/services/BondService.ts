...
import { Bond, BondStatus, BondResolution, BondTemplate } from '../types/Bond';
import { XPTrigger, XPTriggerType } from '../types/XP';
...
export class BondService {
  ...
  private generateBondId(): string {
    return `bond-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`;
  }
