...
import { getAdvancedMovesForClass, getAdvancedMovesAtLevel } from '../data/advancedMoves';
...
class MoveIndexService {
  ...
  async refreshCustomContent(): Promise < void> {
    ...
    const customMoveIds = Array.from(this.moveIndex.keys()).filter(id =>
      this.moveIndex.get(id)?.source === 'Custom',
    );
    customMoveIds.forEach(id => this.moveIndex.delete(id));
  ...
  private findCrossReferences(move: Move): string[] {
    ...
    this.moveIndex.forEach((entry, id) => {
  ...
  async searchMoves(
    ...
    let results = Array.from(this.moveIndex.values());
    ...
    if (query.trim()) {
      ...
      if (this.searchIndex.has(query.toLowerCase())) {
        this.searchIndex.get(query.toLowerCase())!.forEach(id => {
          matchingIds.add(id);
        });
      }
  ...
  async getMovesByClass(characterClass: string): Promise < MoveIndexEntry[]> {
    ...
    return Array.from(this.moveIndex.values())
