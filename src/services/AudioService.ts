...
export class AudioService {
  ...
  private async initializeAudioContext(): Promise<void> {
    try {
      ...
      console.warn('AudioService: Failed to initialize audio context', error)
  ...
  private getSoundCategory(soundKey: string): string {
    ...
    if (soundKey.includes('outcome') || soundKey.includes('magical')) return 'ui'
  ...
  playDiceAudio(data: DiceAudioData): void {
    ...
    if (!sound) {
      console.warn(`AudioService: Sound not found: ${soundKey}`)
      return
    }
  ...
  private playSound(soundKey: string, volume: number): void {
    ...
    try {
      ...
      console.warn(`AudioService: Failed to play sound ${soundKey}`, error)
    }
  }
  ...
  testAudio(): void {
    if (!this.config.enabled) {
      console.warn('AudioService: Audio is disabled')
      return
    }
    
    console.log('AudioService: Testing audio system...')
...
export const playDiceRoll = (force: number, position?: THREE.Vector3) => {
...
export const playDiceCollision = (intensity: number, position: THREE.Vector3, material = 'felt') => {
  audioService.playDiceAudio({ 
    event: 'dice_collision', 
    intensity, 
    position, 
...
export const playDiceSettle = (material = 'ivory', position?: THREE.Vector3) => {
  audioService.playDiceAudio({ 
    event: 'dice_settle', 
    material, 
    position 
...
export const playOutcome = (outcome: 'success' | 'partial' | 'failure') => {
  audioService.playDiceAudio({ 
...
export const playMagicalEffect = (position?: THREE.Vector3) => {
  audioService.playDiceAudio({ 
