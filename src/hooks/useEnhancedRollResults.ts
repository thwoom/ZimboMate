...
export function useEnhancedRollResults(): UseEnhancedRollResultsReturn {
  const [currentResult, setCurrentResult] = useState<EnhancedRollResult | null>(
    null,
  )
    ...
    (
      ...
      if (effectiveCharacterId) {
        ...
        if (automaticConsequences.length > 0) {
          setTimeout(() => {
            gameLogicService.applyConsequences(
              fullResult.id,
              automaticConsequences,
            )
  ...
  const applyConsequences = useCallback(
    (rollId: string, selectedConsequences?: string[]) => {
          ...
          if (roll.id === rollId) {
            return {
              ...
              consequences: roll.consequences.map((c) => ({
                ...c,
                applied: selectedConsequences?.includes(c.id)
                  ? true
                  : c.applied,
              })),
            }
...
export function createEnhancedBasicRoll(
  dice: number[],
  modifier: number = 0,
...
export function createEnhancedAttributeRoll(
  attribute: string,
  dice: number[],
  modifier: number,
): Omit<RollResult, 'id' | 'timestamp'> {
...
export function createEnhancedDamageRoll(
  weaponName: string,
  dice: number[],
): Omit<RollResult, 'id' | 'timestamp'> {
...
export function createEnhancedMoveRoll(
  moveName: string,
  dice: number[],
  modifier: number,
  _moveId?: string,
