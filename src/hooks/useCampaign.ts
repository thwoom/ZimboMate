...
export function useCampaign(): UseCampaignReturn {
  ...
  const { characters: _characters, getCharacter } = useCharacterStore()
  ...
  const journalEntries = useMemo(
    () => currentCampaign?.journalEntries || [],
    [currentCampaign],
  )
  ...
  const campaignStats = useMemo((): CampaignStats => {
    ...
    const totalXPAwarded = sessions.reduce(
      (total, session) => total + (session.xpAwarded || 0),
      0,
    )
  ...
  const getCharacterCampaigns = useCallback(
    (characterId: string) => {
  ...
  const startCampaignSession = useCallback(
    (name: string, characterIds: string[]) => {
      if (!currentCampaign) return
  ...
  const endCampaignSession = useCallback(() => {
    ...
    const lastSession =
      currentCampaign.sessions[currentCampaign.sessions.length - 1]
