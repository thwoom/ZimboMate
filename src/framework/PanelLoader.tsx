...
export async function preloadPanel(panelId: string): Promise<void> {
  ...
  if (!panel) {
    console.warn(`Cannot preload panel "${panelId}": not registered`);
    return;
  }
  ...
  if (panel.component) {
    ...
    try {
      ...
      console.error(`Failed to preload panel "${panelId}":`, error);
    }
  }
}
...
export async function autoPreloadPanels(): Promise<void> {
  ...
  if (panelsToPreload.length > 0) {
    console.log('Preloading panels:', panelsToPreload);
