...
## Level-Up Wizard Regression Checklist

- **Martial flow (stat + move)** – Award XP to a non-caster, run through Overview → Stat → Move → Review, confirm the advanced move appears on the character sheet and the stat bump persists after reload.
- **Caster flow (stat + move + spells)** – Repeat for a wizard or cleric, ensuring the spell step enforces exact selection counts and newly learned spells appear in the spellbook.
- **Multiple pending advancements** – Queue two characters before opening the wizard; verify it walks through each pending entry without losing draft choices.
- **Cancel & resume** – Close the wizard mid-flow, confirm the pending advancement survives via the Resume button and previously selected options are restored.
- Automated coverage lives in `tests/e2e/level-up-wizard.spec.ts`; if the wizard UI changes, update that test alongside refreshed Playwright screenshots.
