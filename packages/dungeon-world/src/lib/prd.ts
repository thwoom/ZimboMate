...
export class PRDParser {
  ...
  constructor(prdPath = 'docs/PRD.md') {
    this.prdPath = resolve(process.cwd(), prdPath.replace(/[^\w/.-]/g, ''))
  }
  ...
  validate(prd: PRD): void {
    ...
    if (!prd.overview)
      throw new Error('PRD overview is required')
    if (!prd.productVision)
      throw new Error('PRD product vision is required')
