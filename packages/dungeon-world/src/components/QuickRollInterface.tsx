...
export const QuickRollInterface: React.FC <QuickRollInterfaceProps> = ({
  ...
  const handleQuickRoll = useCallback(() => {
    ...
    const _roll = diceRollingService.rollStat(
      selectedStat,
      character,
      {
        customModifier,
  ...
  const handleMoveRoll = useCallback((suggestion: MoveSuggestion) => {
    ...
    const _roll = diceRollingService.rollMove(
      suggestion.move,
      character,
      {
        customModifier,
          ...
          <>
            ...
            {mode === 'stat' && (
              <div className="stat-roll-mode">
                <div className="stat-selector">
                  {(Object.keys(character.attributes) as (keyof Attributes)[]).map((item, index) => {
              ...
              <div className={`last-roll-result ${lastRoll.result}`}>
                <div className="roll-summary">
                  ...
                  <span className="roll-breakdown">
                    (
                    {Array.isArray(lastRoll.dice) ? lastRoll.dice.join(' + ') : 'N / A'}
                    {' '}
                    +
                    {' '}
                    {lastRoll.modifier}
            ...
            <div className="keyboard-shortcuts">
              <small>
                ...
                {' '}
                1-6 (select stat), Enter (roll), Tab (switch mode), Esc (close)
              </small>
            </div>
          </>
        )}
