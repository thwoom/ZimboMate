import type { CalculatedValues } from '../services/CalculationEngine'
...
export function useOptimizedCalculations(): OptimizedCalculationResult {
  ...
  const changedFields = useMemo(() => {
    ...
    if (prevChar.hp.current !== currChar.hp.current)
      changes.push('hp')
    if (prevChar.xp !== currChar.xp)
      changes.push('xp')
    if (JSON.stringify(prevChar.attributes) !== JSON.stringify(currChar.attributes))
      changes.push('attributes')
    if (JSON.stringify(prevChar.debilities) !== JSON.stringify(currChar.debilities))
      changes.push('debilities')

    // Check inventory changes
    const prevInv = prev.inventories[prev.activeCharacterId]
    const currInv = state.inventories[state.activeCharacterId]

    if (prevInv && currInv && prevInv.lastUpdated !== currInv.lastUpdated)
      changes.push('inventory')

    // Check modifier changes
    if (prev.modifiers.modifiers.length !== state.modifiers.modifiers.length)
