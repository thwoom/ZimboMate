...
export class PanelDiagnostics {
  ...
  async quickFix(): Promise < void> {
    ...
    for (const key of corruptedStates) {
  ...
  injectDiagnosticTools(): void {
    ...
    if (!isDebugMode) {
      console.log('🔍 Panel Diagnostic Tools disabled. To enable: localStorage.setItem("zimbomate-debug-mode", "true")');
      return;
    }
    ...
    `;
        ...
        console.log('Panel Diagnostic Report:', report);
