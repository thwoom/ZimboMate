...
import type { DiceRoll } from './DiceRollingService'
...
export class SmartMoveSuggestionService {
  ...
  getSuggestions(
    ...
    const detectedSituation = contextDescription
      ? this.detectSituation(contextDescription)
      : situation
  ...
  private analyzeCharacter(character: Character, recentRolls: DiceRoll[]): CharacterAnalysis {
    ...
    const preferredMoves = [...moveUsage.entries()]
      .sort((a, b) => b[1] - a[1])
      .slice(0, 3)
      .map(([moveId]) => moveId)
    ...
    if (this.getStatModifier(stats.CON) < 0)
      missingEssentials.push('Low Constitution-consider defensive moves')
    if (this.getStatModifier(stats.WIS) < 0)
      missingEssentials.push('Low Wisdom-be careful with perception')
  ...
  private extractEnvironmentalFactors(description?: string): string[] {
    ...
    if (lowerDesc.includes('dark'))
      factors.push('darkness')
    if (lowerDesc.includes('loud') || lowerDesc.includes('noise'))
      factors.push('noise')
    if (lowerDesc.includes('crowd'))
      factors.push('crowded')
    if (lowerDesc.includes('trap'))
      factors.push('trapped')
    if (lowerDesc.includes('magic'))
      factors.push('magical')
