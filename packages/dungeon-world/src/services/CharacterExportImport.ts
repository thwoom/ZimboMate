...
import { CharacterTemplate } from './CharacterTemplateService';
...
class CharacterExportImportService {
  ...
  private validateCharacterData(data: any): { isValid: boolean; errors: string[]; warnings: string[] } {
    ...
    // Required fields
    if (!data.id) errors.push('Missing character ID');
    if (!data.name) errors.push('Missing character name');
    if (!data.class) errors.push('Missing character class');
    if (!data.race) errors.push('Missing character race');
    if (!data.attributes) errors.push('Missing character attributes');
    if (!data.level) errors.push('Missing character level');
  ...
  private validateTemplateData(data: any): { isValid: boolean; errors: string[]; warnings: string[] } {
    ...
    // Required fields
    if (!data.id) errors.push('Missing template ID');
    if (!data.name) errors.push('Missing template name');
    if (!data.characterClass) errors.push('Missing character class');
    if (!data.race) errors.push('Missing character race');
    if (!data.attributes) errors.push('Missing attributes');
