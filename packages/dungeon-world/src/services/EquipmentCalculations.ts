...
class EquipmentCalculationService {
    ...
    for (const item of equippedItems) {
      if (isArmor(item)) {
        ...
        if (typeof armorValue === 'number') {
          ...
          if (match) {
            totalArmor += Number.parseInt(match[1])
          }
        }
      }
    }
  ...
  getSpecialEffects(equippedItems: Item[]): EquipmentEffect[] {
    ...
    for (const item of equippedItems) {
      ...
      if (hasTag(item, 'magical')) {
        const enhancement = isWeapon(item)
          ? item.enhancement
          : isArmor(item)
            ? item.enhancement
  ...
  private determineWeaponRange(tags: string[]): 'hand' | 'close' | 'reach' | 'near' | 'far' {
    if (tags.includes('far'))
      return 'far'
    if (tags.includes('near'))
      return 'near'
    if (tags.includes('reach'))
      return 'reach'
    if (tags.includes('close'))
      return 'close'
    return 'hand'
  }
  ...
  private getWeaponType(weapon: Item): string {
    if (hasTag(weapon, 'two-handed'))
      return 'heavy'
    if (hasTag(weapon, 'close') || hasTag(weapon, 'reach'))
      return 'martial'
    return 'simple'
  }
  ...
  private scoreWeapon(weapon: Item, character: Character): number {
    ...
    if (isWeapon(weapon) && weapon.damage) {
      ...
      if (damageMatch) {
        const dice = Number.parseInt(damageMatch[1])
        const sides = Number.parseInt(damageMatch[2])
        score += dice * sides
      }
    }
