...
import { CircleHelp, BookOpen, Keyboard, TriangleAlert, Search, Rocket, ChevronDown, ExternalLink, Copy, Check } from 'lucide-react'
import { Card, CardContent, Button, Input, Badge } from './index'
...
export const HelpSystem: React.FC<HelpSystemProps> = ({ 
  ...
  const renderContent = (content: any[]) => {
    return content.map((item, index) => (
      ...
      >
        {item.type === 'heading' && (
          <h3 className="text-display-sm mb-4 mt-8 first:mt-0" style={{ color: 'var(--color-primary)' }}>
            {item.content}
          </h3>
        )}
        
        {item.type === 'subheading' && (
          <h4 className="text-body-lg font-semibold mb-3 mt-6" style={{ color: 'var(--color-text-primary)' }}>
            {item.content}
          </h4>
        )}
        
        {item.type === 'paragraph' && (
          <p className="text-body-regular mb-4 leading-relaxed" style={{ color: 'var(--color-text-secondary)' }}>
            {item.content}
          </p>
        )}
        ...
        {item.type === 'list' && (
          <ul className="list-disc list-inside mb-4 space-y-2">
            {item.items.map((listItem: string, i: number) => (
              <li key={i} className="text-body-regular" style={{ color: 'var(--color-text-secondary)' }}>
                {listItem}
              </li>
            ))}
          </ul>
              ...
              <thead>
                <tr style={{ borderBottom: '2px solid var(--color-border)' }}>
                  {item.headers.map((header: string, i: number) => (
                    <th key={i} className="text-left p-3 font-semibold" style={{ color: 'var(--color-text-primary)' }}>
                      {header}
                    </th>
                  ))}
                </tr>
              ...
              <tbody>
                {item.rows.map((row: string[], i: number) => (
                  <tr key={i} style={{ borderBottom: '1px solid var(--color-border)' }}>
                    {row.map((cell, j) => (
                      <td key={j} className="p-3 text-body-regular" style={{ color: 'var(--color-text-secondary)' }}>
                        {cell.includes('**') ? (
                          <code 
        ...
        {item.type === 'code' && (
          ...
          >
            <code className="text-sm font-mono" style={{ color: 'var(--color-primary)' }}>
              {item.content}
            </code>
            <div className="float-right mt-1">
              {copiedText === item.content ? (
                ...
                <Copy size={16} className="opacity-50" style={{ color: 'var(--color-text-muted)' }} />
              )}
            </div>
          </pre>
        )}
        ...
        {item.type === 'callout' && (
          <div 
            ...
            style={{ 
              backgroundColor: item.variant === 'warning' ? 'var(--color-warning-light)' : 'var(--color-primary-light)',
              borderLeftColor: item.variant === 'warning' ? 'var(--color-warning)' : 'var(--color-primary)'
            }}
          >
            <p className="text-body-regular font-medium" style={{ color: 'var(--color-text-primary)' }}>
              {item.content}
            </p>
          </div>
        )}
    ...
    if (!filteredContent || filteredContent.length === 0) {
      return (
        <div className="text-center py-12">
          <Search size={48} className="mx-auto mb-4 opacity-50" style={{ color: 'var(--color-text-muted)' }} />
          <p className="text-body-regular" style={{ color: 'var(--color-text-muted)' }}>
    ...
    <Card variant="magical" padding="lg">
      <CardContent>
        <div className="flex items-center justify-between mb-6">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-body-regular" style={{ color: 'var(--color-text-secondary)' }}>
        ...
        <div className="relative mb-6">
          <Search size={20} className="absolute left-3 top-1/2 transform -translate-y-1/2" style={{ color: 'var(--color-text-muted)' }} />
          <Input
        ...
        {searchQuery.trim() ? (
          <div>
            <div className="flex items-center gap-2 mb-4">
              ...
              <Badge variant="secondary">{filteredContent?.length || 0} results</Badge>
              <Button
                variant="ghost"
                size="sm"
              ...
              {helpSections.map(section => {
                ...
                return (
                  <Tabs.Trigger
                    ...
                    style={{
                      backgroundColor: activeTab === section.id ? 'var(--color-primary)' : 'transparent',
                      color: activeTab === section.id ? 'white' : 'var(--color-text-secondary)'
                    }}
                  >
                    <Icon size={16} />
