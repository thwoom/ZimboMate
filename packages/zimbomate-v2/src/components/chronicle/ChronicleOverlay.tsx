...
const ChroniclePromptCard: React.FC<{
  ...
  const getPriorityIcon = () => {
    switch (prompt.priority) {
      case 'high': return <Zap size={16} className="text-orange-500" />
      case 'medium': return <Target size={16} className="text-blue-500" />
      case 'low': return <Clock size={16} className="text-gray-500" />
    }
  }
  ...
  return (
    <motion.div
      ...
      className={`
        ...
        ${prompt.priority === 'high' ? 'ring-2 ring-orange-200 dark:ring-orange-800' : ''}
      `}
    >
      {/* Header */}
