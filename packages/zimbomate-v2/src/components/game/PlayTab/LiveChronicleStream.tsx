...
const EntryTypeIcon: React.FC<{ type: string }> = ({ type }) => {
  switch (type) {
    ...
    case 'dialogue':
      return <Quote size={12} className="text-blue-500" />
    case 'discovery':
      return <Sparkles size={12} className="text-yellow-500" />
    case 'reaction':
      return <User size={12} className="text-green-500" />
    default:
      return <BookOpen size={12} className="text-gray-500" />
  }
}
  ...
  return (
    <motion.div
      ...
      className={`p-3 rounded-lg border border-gray-200 dark:border-gray-700 ${
        entry.isAutoSuggested ? 'bg-blue-50 dark:bg-blue-950/20' : 'bg-white dark:bg-gray-800'
      }`}
    >
...
const QuickAddForm: React.FC<{
  ...
  const typeButtons = [
    { id: 'action', label: 'Action', icon: Zap, color: 'text-orange-500' },
    { id: 'dialogue', label: 'Dialogue', icon: Quote, color: 'text-blue-500' },
    { id: 'discovery', label: 'Discovery', icon: Sparkles, color: 'text-yellow-500' },
    { id: 'reaction', label: 'Reaction', icon: User, color: 'text-green-500' }
  ]
    ...
    <div className="space-y-3">
      ...
      {suggestions.length > 0 && (
        <div className="space-y-2">
          <div className="text-xs text-gray-600 dark:text-gray-400">Quick suggestions:</div>
          <div className="flex flex-wrap gap-1">
            {suggestions.slice(0, 3).map(suggestion => (
              <Button
                ...
                className="text-xs px-2 py-1 h-auto bg-blue-50 dark:bg-blue-950/20 hover:bg-blue-100 dark:hover:bg-blue-900/30"
              >
      ...
      <form onSubmit={handleSubmit} className="space-y-2">
        ...
        <div className="relative">
          <textarea
            ...
            className="w-full p-2 text-sm border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-800 resize-none"
            rows={2}
            onKeyDown={(e) => {
          ...
          <Button
            ...
            className={`absolute top-2 right-2 w-6 h-6 p-0 ${isListening ? 'text-red-500 animate-pulse' : 'text-gray-400'}`}
          >
          ...
          {!autoScroll && (
            ...
            >
              <Button
                ...
                className="bg-white dark:bg-gray-800 shadow-lg gap-1"
              >
                <ArrowDown size={12} />
                New entries
              </Button>
