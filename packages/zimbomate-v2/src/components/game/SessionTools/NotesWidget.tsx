...
export const NotesWidget: React.FC<NotesWidgetProps> = ({ 
  ...
  const getImportanceIcon = (importance: Note['importance']) => {
    switch (importance) {
      case 'critical':
        return <Star size={12} className="text-red-500 fill-current" />
      case 'important':
        return <Star size={12} className="text-yellow-500 fill-current" />
      default:
        return <Star size={12} className="text-gray-400" />
    }
  }
                    ...
                    {note.tags.length > 0 && (
                      <div className="flex flex-wrap gap-1">
                        {note.tags.map((tag, tagIndex) => (
                          <Badge key={tagIndex} variant="secondary" className="text-xs gap-1">
                            <Tag size={10} />
                            {tag}
                          </Badge>
                        ))}
            ...
            <div className="grid grid-cols-4 gap-4 text-center text-sm">
              ...
              <div>
                <div className="font-medium text-red-600">
                  {sessionNotes.filter(n => n.importance === 'critical').length}
                </div>
                <div style={{ color: 'var(--color-text-secondary)' }}>Critical</div>
              </div>
              <div>
                <div className="font-medium text-yellow-600">
                  {sessionNotes.filter(n => n.importance === 'important').length}
                </div>
                <div style={{ color: 'var(--color-text-secondary)' }}>Important</div>
              </div>
              <div>
                <div className="font-medium">
                  {new Set(sessionNotes.flatMap(n => n.tags)).size}
                </div>
                <div style={{ color: 'var(--color-text-secondary)' }}>Tags</div>
              </div>
            </div>
          </CardContent>
        </Card>
