...
const getDispositionIcon = (disposition: string) => {
  switch (disposition) {
    case 'friendly': return <Heart size={16} className="text-green-500" />
    case 'neutral': return <Shield size={16} className="text-blue-500" />
    case 'hostile': return <Skull size={16} className="text-red-500" />
    case 'unknown': return <HelpCircle size={16} className="text-gray-500" />
                  ...
                  {formData.secrets.length > 0 && (
                    <div className="space-y-2">
                      {formData.secrets.map((secret, index) => (
                        <div key={index} className="flex items-start gap-2 p-3 bg-gray-500/10 rounded-lg">
