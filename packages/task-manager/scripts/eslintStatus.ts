...
function runESLint(): ESLintResult {
  try {
    ...
    const totalProblems = summaryMatch ? Number.parseInt(summaryMatch[1]) : 0;
    const errors = summaryMatch ? Number.parseInt(summaryMatch[2]) : 0;
    const warnings = summaryMatch ? Number.parseInt(summaryMatch[3]) : 0;
    const fixable = fixableMatch ? Number.parseInt(fixableMatch[1]) : 0;
